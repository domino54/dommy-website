<?php
	...
	$pageTitleMeta = 'Dommy - Servers list';
	$pageDescriptionMeta = 'List of my ManiaPlanet servers';
	$headerTitleText = 'Servers list';
	...
	/**
	 *	Get the time of the latest request
	 */
	$serversUpdateTimeCacheFile = CACHE_DIRECTORY.'servers-update-time.txt';
	$serversDataArrayCacheFile = CACHE_DIRECTORY.'servers-serialized-data.txt';
	$lastServersUpdateTime = -1;
	if (file_exists($serversUpdateTimeCacheFile))
		$lastServersUpdateTime = intval(file_get_contents($serversUpdateTimeCacheFile));
	 ...
	 */
	if (time() >= $lastServersUpdateTime + CACHE_INTERVAL || !file_exists($serversDataArrayCacheFile)) {
		$lastServersUpdateTime = time();
		file_put_contents($serversUpdateTimeCacheFile, $lastServersUpdateTime);
		 ...
		 */
		try {
			foreach ($ManiaPlanet->getFilteredList(array('titleUids' => 'GalaxyTitles@domino54')) as $server) loadServer($server);
		}
		catch (\Maniaplanet\WebServices\Exception $e) {
			$exceptionMessage .= MP_WS_EXCEPTION_PREFIX.$e->getMessage();
		}
		...
		file_put_contents($serversDataArrayCacheFile, serialize($savedServersData));
	}
	/**
	 *	Load previous data from cache
	 */
	else $savedServersData = unserialize(file_get_contents($serversDataArrayCacheFile));
	...
	else if ($displayedServers <= 0) $errorMessage = 'Looks like there are no servers online at the moment!';
	 ...
	 */
	$textPageContent = '
<div id="subpage-description">Here you can find all ManiaPlanet servers running game modes created by me</div>
<div style="margin: 8px;">
	<div id="servers-list-container">
	'.$serversListText.'
	</div>
</div>
<div id="servers-update-time">Last update '.(time() - $lastServersUpdateTime).' second(s) ago.</div>
	';
?>